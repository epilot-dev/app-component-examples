...
export const AddressMap = ({ street, number, city, center, addCoordsToMapping }: Props) => {
  ...
  // Geocode address only when all fields are present; otherwise reset to Germany view
  useEffect(() => {
    const allPresent = Boolean(street && number && city);
    if (!allPresent) {
      setCoords(null);
      if (mapRef.current) {
  ...
  // When coords are available, update view and marker
  useEffect(() => {
    if (!mapRef.current) return;

    if (!coords) {
      // No specific coordinates; ensure no marker
      if (markerRef.current) {
        markerRef.current.remove();
        markerRef.current = null;
      }
